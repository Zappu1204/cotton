...
def train_val_split(data_folder, output_folder):
    ...
    inference_file = os.path.join(output_folder, 'inference.txt')
    ...
    f = open(inference_file,"w")
    for idx in inference:
        reverse_idx = len(inference) - idx - 1
        f.write("{}.jpg {}.jpg\n".format(idx, reverse_idx))
    f.close()
...
def build_dataset(opt):
    ...
    new_dataset = os.path.join(Data_path, 'Training_Dataset/{}x{}'.format(opt.h, opt.w))
